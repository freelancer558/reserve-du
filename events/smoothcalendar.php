...
class SmoothCalendar {
    ...
    public function listreserveByGroupId($from, $to) 
    {
        ...
        if (isset($to))
        {
            $sql = "SELECT * FROM `reserve` WHERE `Date` >= '" . date('Y-m-d', $from) . "' AND `Date` <= '" . date('Y-m-d', $to) . "' AND `show` = true GROUP BY `group_id` DESC";
          $product = "SELECT * FROM `reserve` WHERE `Date` >= '" . date('Y-m-d', $from) . "' AND `Date` <= '" . date('Y-m-d', $to) . "' AND `show` = true ORDER BY `group_id` DESC";
        }
        else
        {
           $sql = "SELECT * FROM `reserve` WHERE `Date` >= '" . date('Y-m-d', $from) . "' AND `show` = true GROUP BY `group_id` DESC";
           $product = "SELECT * FROM `reserve` WHERE `Date` >= '" . date('Y-m-d', $from) . "' AND `show` = true ORDER BY `group_id` DESC";
        }
 ...
 public function updatereserveStatus($event)

    {
        ...
        $q_overlap = $this->query("SELECT * FROM `reserve` WHERE `DurableArticleSet` = '". trim($found['DurableArticleSet']) ."' AND status = 'approved' AND Date BETWEEN DATE('".$found['Date']."') AND DATE('".$found['end_date']."')");
  ...
  $num_overlap = mysql_num_rows($q_overlap);
    ...
    public function createEvent($event)
    {
        if (!$this->options["create"])
            ...
            for($i=0; $i<count($event["content"]); $i++){
                ...
                // return $this->checkDateTimeOverlap($content, $date, $end_date);
                 ...
                 return $this->throw_server_exception("มีการใช้งานเครื่องมือในช่วงเวลานั้นแล้ว หรือ ช่วงเวลาที่เลือกไม่ถูกต้อง");
        ...
        return array("message" => "ทำการส่งคำร้องการจองสำเร็จ");
    }
