(function($){
    ...
    $.fn.smoothPhpCalendar = function(opts){
		...
		function onOpenningDayEventAnimationComplete(options, dayOfMonth) 
		{
			...
			eventListContainer.append('<table class="table table-striped"><thead><th>ชื่อผู้ใช้</th><th>วันที่จอง</th><th>เวลาที่จอง</th><th>วันที่สิ้นสุดการจอง</th><th>เวลาที่สิ้นสุดการนจอง</th><th>เครื่องมือ</th></thead><tbody></tbody></table>')
			...
			for (var i = 0; i < eventsOfDay.length; i++)
			{
				...
				eventListContainer.find('tbody').append(
					...
					$('<td><p class=event-time></p></td>').html('end at: '+getTimeOfEvent(e.end_date)),
					$('<td><p class=event-time></p></td>'   ).html('start at: '+getTimeOfEvent(e.date)),
					$('<td><p class=event-time></p></td>').html('end at: '+getTimeOfEvent(e.end_date)),
