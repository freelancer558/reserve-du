...
<center> เครื่องมือ : pH meter <br>
  ...
  ?>
  <select name="select" class="setname" onChange="namosw_goto_byselect(this, 'self')">
  <?php
    while($tools = mysql_fetch_assoc($q)){
  ?>
    <option value="index.php?filter=<?php echo $tools['DurableArticleSetEnglishName'];?>" <?php echo (isset($_GET['filter']) && $_GET['filter'] == $tools['DurableArticleSetEnglishName']) ? 'selected' : ''; ?> ><?php echo $tools['DurableArticleSetEnglishName'];?></option>
  <?php
    }
  ?>
  </select>
