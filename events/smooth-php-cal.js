		...
		function getWeekDaysRow(el, options)
		{
			...
			while (actualPrintedDays != totalDaysInMonth && currentPrintedDays != (totalDaysInMonth + firstDayOfMonth + Math.ceil(totalDaysInMonth / 7))) 
			{
				...
				if (currentPrintedDays >= firstDayOfMonth && actualPrintedDays <= totalDaysInMonth - 1) 
				{
					...
					var eventDay   = getEventsOfDay(options, dayNumber);
					var year        = options.currentDate.getFullYear();
					var month       = options.currentDate.getMonth();
					var day 		= options.currentDate.getDate();
					var eventList  = "";
					for(var i=0; i<eventDay.length; i++){
						if(i<=1) eventList += "<p class='day_event_list' data-id='"+eventDay[i]['id']+"'></p>";
					}
					// console.log(getEventsOfDay(options, dayNumber));
		...
		function onOpenningDayEventAnimationComplete(options, dayOfMonth) 
		{
			...
			with (options.dateBeingViewed) 
				var dayText = options.weekDays[getDay()] + ' ' + 
							  ...
							  getDate() + ', ' + 
							  getFullYear();
			var startAt = new Date(options.startDate).getDate()+'/'+new Date(options.startDate).getMonth()+'/'+new Date(options.startDate).getFullYear();
			...
			var reservBtn 	   = $("<p><a href='event-create.php?start_at="+startAt+"'></a></p>").addClass('reservBtn');
			var div = $('<div></div>').css({
			...
			eventListContainer.append('<table class="table table-striped"><thead><th>ชื่อผู้ใช้</th><th>วันที่จอง</th><th>เวลาที่จอง</th><th>วันที่สิ้นสุดการจอง</th><th>เวลาที่สิ้นสุดการจอง</th><th>เครื่องมือ</th></thead><tbody></tbody></table>')

			for (var i = 0; i < eventsOfDay.length; i++)
			{
				var e = eventsOfDay[i];
				eventListContainer.find('tbody').append(
					'<tr>',
					$('<td><p class=event-username></p></td>').html(e.fullname),
					$('<td><p class=event-time></p>,</td>'   ).html(e.date),
					$('<td><p class=event-time></p></td>'   ).html('เริ่มเวลา: '+getTimeOfEvent(e.date)),
					$('<td><p class=event-time></p></td>'   ).html(e.end_date),
					$('<td><p class=event-time></p></td>').html('สิ้นสุดเวลา: '+getTimeOfEvent(e.end_date)),
					$('<td><p class=event-details></p></td>').html(e.content),
					'</tr>'		
				);
			}
